...
async def main():
    ...
    # Enrich market_cap using HISTORICAL prices on the ingest date.
    # Uses yfinance get_shares_full × close on args.date.
    log(f"\nEnriching historical market_cap for {len(touched_companies)} unique companies on {args.date}...")
    mcap_filled = 0
    mcap_skipped = 0
    mcap_errored = 0
    for cik, ticker in touched_companies.items():
        mcap = StockPriceService.get_market_cap_at_date(ticker, args.date)
    ...
    for cik in touched_companies.keys():
        try:
            result = await InsiderClusterService.process_incremental(cik, args.date, historical=True)
        except Exception as e:
            errored += 1
