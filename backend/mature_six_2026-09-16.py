"""Mature the six strong_buy SignalPerformance rows now past day 90.

SMBK, INR, MBC, NEWT, XRAY, CXT. Cloned verbatim from
mature_gshd_rli_2026-09-01.py — only TARGET_TICKERS and EXPECTED_ROWS differ.
That script already derives day-90 PER ROW from actionable_date, so the six
different actionable dates need no other change:

    SMBK 2026-06-10 -> day-90 2026-09-08
    INR  2026-06-12 -> day-90 2026-09-10
    MBC  2026-06-12 -> day-90 2026-09-10
    NEWT 2026-06-15 -> day-90 2026-09-13 (Sun -> forward to Mon 09-14 close)
    XRAY 2026-06-15 -> day-90 2026-09-13 (Sun -> forward to Mon 09-14 close)
    CXT  2026-06-16 -> day-90 2026-09-14

All six are fully past day 90 as of 2026-09-16, per the maturation-timing rule:
never on day-90 itself. Sunday day-90s resolve forward to the next close via
close_on_or_after — the convention find_price uses in signal_performance_service.

Per feedback_matured_signals_frozen.md: once is_mature=true a row is IMMUTABLE.
Updates are IN PLACE — no DELETE/CREATE — and frozen rows are untouched.

Day-90 prices come from yfinance directly, NOT Company.price_series, so series
staleness cannot affect them. Prices were refreshed 2026-09-16 before this run,
so return_current is same-day and the sanity tolerance is meaningful.

Safety:
- ABORT if not exactly 6 rows load
- ABORT if any required entry price is missing and cannot be fetched
- ABORT if yfinance returns no day-90 / SPY data
- ABORT if computed return_day0 deviates from the row's return_current by more
  than tol = max(20pp, 0.30*|return_current|)
- Rows are processed in order; if ANY row aborts, nothing downstream runs and
  the dashboard is NOT refreshed — but rows already committed stay committed.
...
TARGET_TICKERS = ["SMBK", "INR", "MBC", "NEWT", "XRAY", "CXT"]
EXPECTED_ROWS = 6
