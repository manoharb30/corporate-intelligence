"""Mature the HNRG strong_buy SignalPerformance row.

Cloned from mature_six_2026-09-16.py — only TARGET_TICKERS and EXPECTED_ROWS
differ. That script derives day-90 PER ROW from actionable_date, so nothing else
needs changing.

    HNRG actionable 2026-06-25 -> day-90 2026-09-23 (Wed)

TIMING: day-90 was 2026-09-23. Per feedback_maturation_timing this must run only
AFTER that date has fully passed in US EASTERN time — never on the day itself,
even with markets closed (ALT hit day 90 on Sunday 2026-07-05 and was still
deferred to the next day). So: run on or after 2026-09-24 ET.

Expect roughly -20%. HNRG is the first signal to mature with a TRIGGERED BREAK
CONDITION already on its Signal Watch ledger: the 2026-05-06 call promised "a
clean summer run after planned outage" with "a second availability miss" set as
the break. That miss happened — unplanned Merom Unit 2 downtime during
high-price periods on top of the planned 60-day Unit 1 outage, forcing
replacement power purchases in MISO, and electric segment EBITDA collapsing
$41.8M -> $2.2M with a $15.2M quarterly net loss. The thesis was called broken in
August and the price agreed. Its other three promises passed (Siemens turbine
FID, IURC approval still tracking, forward book grown to ~$2.37B), but none
offset the plant not running.

Per feedback_matured_signals_frozen: once is_mature=true the row is IMMUTABLE.
Updates are IN PLACE — no DELETE/CREATE.

Day-90 price comes from yfinance directly, not Company.price_series, so series
staleness cannot affect it. The 2026-09-23 close was confirmed present at
$14.51 during the price refresh that evening.

Safety:
- ABORT if not exactly 1 row loads
- ABORT if any required entry price is missing and cannot be fetched
- ABORT if yfinance returns no day-90 / SPY data
- ABORT if computed return_day0 deviates from return_current by more than
  tol = max(20pp, 0.30*|return_current|)

Default = COMMIT. Pass --dry-run to compute and report without writing.
"""
import asyncio
import sys
