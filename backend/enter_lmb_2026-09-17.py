"""One-off portfolio entry: LMB, 2026-09-17, per the locked paper-portfolio spec.

LMB signalled 2026-09-16 (day-0 $52.56) and was KEPT on per-arrival review:
two directors buying meaningfully — Gaboury $99,619, Horowitz $49,590 then
ADDING $10,739 — into a -54% de-rating, on the cheapest EV/EBITDA in a strong
peer group with a 6% FCF yield. See the ResearchNote of 2026-09-17.

Cloned from enter_dfh_2026-09-16.py, which is itself enter_uuuu plus SGOV funding.
Cash rests at the $1,000 buffer, so this liquidates ~$5,000 of SGOV before buying.
NOTE: LMB is one of TWO entries on 2026-09-17 (the other is the sibling script),
so ~$10K of SGOV is sold across the pair — comfortable against ~$94K of sweep, but
run them in sequence, not concurrently, so the funding sales cannot race.

Entry day is the day the signal SURFACED (detection + review), never the signal
date — day0 below is the shortfall BASIS only, not a target. The signal surfaced
in the 2026-09-16 backfill; the market was already closed when review finished,
so per the spec an after-hours signal enters on the next trading day.

Resumable: counts today's already-filled LMB buys on start and skips that many.

Run detached inside the backend container:
    docker exec -d lookinsight-backend python enter_lmb_2026-09-17.py --commit
Log: /app/backend/enter_lmb_2026-09-17.log (inside container)
...
SIGNAL_ID = "CLUSTER-0001606163-2026-09-16"
SIGNAL_DATE = "2026-09-16"
DAY0_PRICE = 52.56
EXIT_DATE = "2026-12-15"  # day-90; resolves forward at exit time if non-trading

LOG_PATH = Path(__file__).parent / "enter_lmb_2026-09-17.log"
