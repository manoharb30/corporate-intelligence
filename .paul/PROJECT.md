...
| Earnings-proximity rule applies to unmatured signals only | v1.7 Phase 17 decision. The `≤60d to next earnings` rule in `merge_classifications.py` continues to demote GENUINE → FILTERED. Phase 18's classification filter addition will exclude FILTERED rows from cluster detection, enforcing the rule properly for future and currently-immature signals. **Matured 142 cohort is frozen per v1.2 invariant — not re-classified against the enforced rule.** v1.1 brief numbers stay as shipped; v1.2+ briefs use the clean cohort. | 2026-04-23 | Active |
| Matured signals are frozen at methodology level (not just code level) | Extends v1.2 immutability from "don't mutate SignalPerformance rows" to "don't revisit methodology of matured signals at all." Matured cohort reflects the rules that produced it and stays that way. New rules only apply to future signals. Durable principle for all milestone work. | 2026-04-23 | Active |
